...
import { Search, SlidersHorizontal, LayoutGrid, Rows3, ChevronDown, Sparkles, Type } from 'lucide-vue-next'
...
async function runSearch(reset = true) {
  if (!isAuthenticated.value && mode.value !== 'lexical') mode.value = 'lexical'
...
const displayItems = computed(() => search.results)
const displayLoading = computed(() => search.loading)
...
<template>
  <div class="space-y-8 md:space-y-10">
        ...
        <form @submit.prevent="runSearch(true)" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <div class="inline-flex items-center rounded-md border bg-card p-0.5">
            <button
              ...
              @click="mode = 'lexical'; (hasSearched || filters.name) && runSearch(true)"
              title="词法模糊匹配 · 最快"
            >
            ...
            <button
              ...
              @click="mode = 'hybrid'; (hasSearched || filters.name) && runSearch(true)"
    ...
    <div v-if="!hasSearched && !search.loading && !search.hasResults" class="flex flex-col items-center justify-center py-12 md:py-16 text-center">
