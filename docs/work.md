...
```bash
# 1. 拷贝备份进 postgres 容器
docker cp legacy/postgres_backup.sql malaco-postgres:/tmp/backup.sql

# 2. 创建 staging 表并抽取 COPY 块
docker exec malaco-postgres psql -U malaco -d malacoagent -c "
...
docker exec malaco-postgres psql -U malaco -d malacoagent -c "DROP TABLE shellauction_staging;"
```
