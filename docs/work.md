...
（历史记录：第三方中文数据源集成，已于 2026-05-22 下线）
...
阿里云 ECS 无法访问 ganvana.com（上游 CDN/WAF 拒绝），已于前端和后端清理相关代码。
...
## 中文俗名导入（冈瓦纳词典）(2026-05-23) ✅

从外部中文词典导入 27,700 条中文俗名到 `taxa_vernaculars`。后续升级为
严格全名匹配避免贪婪错误匹配，并增加繁→简自动转换。已于 2026-05-27 随
冈瓦纳服务下线而清理相关代码。
...
**俗名同步策略**：
- 新增 `taxa_vernaculars.source` 列追踪来源（`inaturalist` / `manual`）
