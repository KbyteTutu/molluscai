...
**背景**: `./dev nuke && ./dev up && ./dev prod-import` 从零重建全栈时发现 3 个阻塞性 bug。修复按 guide.md §3 最小改动原则，涉及 3 个文件。

### 修复清单

| # | 问题 | 根因 | 修复 |
|---|------|------|------|
| 1 | `./dev up` 在 `.env` 无 `MINIO_DATA_PATH` 时静默退出 (exit 1) | `env_get()` 内 `grep -E` 未匹配返回 1，`set -euo pipefail` + 命令替换 → 父 shell 直接 kill | `scripts/dev.sh`: `grep ... \|\| true` |
| 2 | 所有 `/api/v1/*` 前端代理返回 404 `{"detail":"Not Found"}` | commit 537a452 引入变量化 `proxy_pass` 但未适配 URI 处理：nginx 在 target 含变量时不 auto-append location tail，所有请求塌缩为 `GET /api/` | `frontend/nginx.conf`: `/api/` 用 `$request_uri` 显式传递原始 URI；`/minio/` 用 `rewrite` 剥前缀后不含 URI proxy_pass |
| 3 | `GET /auction/search` 返回 422 `int_parsing`（"search" 不是合法 int） | 仅有 `POST /auction/search`，`GET` 请求被 `GET /{item_no}` (int) 捕获 | `backend/app/api/v1/auction.py`: 添加 `GET /search` 405 守卫（`include_in_schema=False`，`Allow: POST` 头） |

### 部署验证

| 服务 | 状态 |
|------|------|
| postgres (pgvector) | healthy, auctions=2,990,337 / taxa=315,589 |
| redis | PONG |
| minio | live |
| backend (uvicorn --reload) | /health OK |
| celery-worker | running |
| celery-beat | running |
| frontend (nginx) | 200, /api/v1/* 转发正常 |

### 变更文件

| 文件 | 说明 |
|------|------|
| `scripts/dev.sh` | `env_get()` 缺 key 时优雅返回空字符串 |
| `frontend/nginx.conf` | 变量化 proxy_pass 后显式传递 URI |
| `backend/app/api/v1/auction.py` | `GET /search` → 405 Method Not Allowed |

**提交**: 3 个原子化 commits，已推送 `origin/main`
