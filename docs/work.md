...
| **P1.11 — UX + 查询** | 邮箱登录 + logo 头像 + 侧边栏布局 + 查询日志过滤 + IP 智能显示 + 拍卖全局搜索 + relevance 排序 | ✅ 已完成 |
| **P2 — 知识库核心** | PDF 上传 + 解析 Pipeline（OCR → 元数据 → 切分 → Embedding） | 🔲 待实施 |
| **P3 — RAG 检索** | 混合检索 + 生成回答 + 问答界面 | 🔲 待实施 |
| **P4 — 收费系统** | 计费 + 余额 + 充值 | 🔲 待实施 |
| **P5 — 安全加固** | 限流 + 反爬 + 图片缓存 + HTTPS | 🔲 待实施 |
...
```bash
# 干净机器部署全流程
sudo apt-get install -y python3-pip nodejs npm    # LSP + Playwright 依赖
pip install --user --break-system-packages basedpyright
sudo npm install -g @vue/language-server @playwright/mcp
npx playwright install chromium
# → 然后 ./dev up + ./dev prod-import 即可拥有与生产等量的测试环境
```
