...
| 文件 | 改动 |
|------|------|
| `infra/postgres/init/08-taxon-name-zh.sql` | 新建 `taxon_name_zh(latin_name PK, chinese_name, rank_type)` + rank_type 索引 |
| `backend/scripts/import_taxon_zh.py` | 解析 xlsx：A-H 列按 `rsplit(' ', 1)` 提取 `中文 拉丁`；J/L 列直接组对；UPSERT 写入 |
| `backend/requirements.txt` | 新增 `openpyxl>=3.1` |
...
```bash
# 1. 初次：建表（已直接在运行中 DB 执行）
# 2. 容器装 openpyxl（已写入 requirements.txt，下次 build 自动）
docker compose exec backend pip install openpyxl

# 3. 复制 xlsx 到容器并导入
docker compose exec backend mkdir -p /app/data_import
